...
(defclass City ()
	((hn	   :accessor hn		  :initarg :hn		:initform 0)		;h(n)
	 (name	   :accessor name	  :initarg :name 	:initform "Dummy")	;name
	 (f_s	   :accessor f_s	  :initarg :f_s 	:initform "")		;first successor
	 (f_gn	   :accessor f_gn	  :initarg :f_gn	:initform 0)		;first successor's g(n)
	 (s_s	   :accessor s_s	  :initarg :s_s 	:initform "")		;second successor
	 (s_gn	   :accessor s_gn	  :initarg :s_gn	:initform 0)		;second successor's g(n)
	 (t_s	   :accessor t_s	  :initarg :t_s 	:initform "")		;third successor
	 (t_gn	   :accessor t_gn	  :initarg :t_gn	:initform 0)		;third successor's g(n)
	 (l_s	   :accessor l_s	  :initarg :l_s 	:initform "")		;last successor
	 (l_gn	   :accessor l_gn	  :initarg :l_gn	:initform 0)		;;last successor's g(n)
	)
)
...
(setf R  (make-instance 'City :hn 193 :f_gn  80 :s_gn  97 :t_gn 146           :name "Ramnicu Valcea"))
...
;setting the successor seperately because the successor hasn't exist when it is declared to another instance's successor
(setf (f_s A) S)
(setf (s_s A) Ti)
(setf (t_s A) Z)
...
(defun astar (current)
	(setf travel (concatenate 'string travel (name current) " -> ")) ;creating string of solution path
	(setf new_city ())
	(setf fn (+ (hn current) gn)) ; initial f(n) = g(n) + h(n)
	(setf list_fn (cons fn ()))
	(setf new_city (cons (name current) list_fn))
	(setf fringe (cons new_city fringe))
	; (format t (name current))
	(format t "~%fringe (" (name current))
	(setf fringe (sort fringe #'< :key #'second))
	(dolist (i fringe) (format t " ~s "(car i))) 
	(format t ")")
	(format t "~%Node Expanded : ~a ~%" (name current))
	; (format t "'s f(n) = " )
	; (format t "~a" fn) 

	; (format t "~%Next most minimum f(n) cost = " )
	; (format t "~a" 
			 	   (setf min_fn (min (if (= (f_gn current) 0) 9999 (+ (hn (f_s current)) (f_gn current))) 
	...
	(if (equal "Bucharest" (name next)) 
		(format t "~%Total Distance Traveled = ~a~%Solution Path: ~aBucharest"gn travel) 
		(astar next)
	)
)

(astar A) ; the function starter
