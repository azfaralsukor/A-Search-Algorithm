...
; (defun search ()
	; (setf f1 (+ (hn current) (f_gn current)))
	; (setf f2 (+ (hn current) (s_gn current)))
	; (setf f3 (+ (hn current) (t_gn current)))
	; (setf f4 (+ (hn current) (l_gn current)))

	; (print (min f1 f2 f3 f4))
	; (format t "Cuboid or Sphere ? (in double quotes): ")
	; (setq input (read))
	; (cond
	; 	((equal input "cuBoid")
	; 		(progn 
	; 			(format t "Enter the values of l, w and h in a list (l w h): ")
	; 			(setq c (read))
	; 			(+ (* 2 (car c) (cadr c)) (* 2 (cadr c) (caddr c)) (* 2 (caddr c) (car c)))
	; 		)
	; 	)
	; 	((equal input "sPHeRe")
	; 		(progn 
	; 			(format t "Key in value of r in a list (r):")
	; 			(setq s (read))
	; 			(* (expt (car s) 2) pi 4)
	; 		)
	; 	)
