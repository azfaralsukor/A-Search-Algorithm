...
(print (setf fn (+ (hn A) 0 ))) ; initial f(n) for Arad
(setf current A)				; set Arad to current city
; (setf first (f_s current))
; (describe A)
...
; (dotimes (n  3) 
  	; (print  n) (prin1 (* n  n))
  	(print (min (+ (hn (f_s current)) (f_gn current)) 
  				(+ (hn (s_s current)) (s_gn current)) 
  				(+ (hn (t_s current)) (t_gn current)) 
  				(+ (hn (l_s current)) (l_gn current))
  			)
  	)
; )
